...
print_editor :: proc(label: string, id: cstring) {
	text := string(gmte_get_text(id))
	fmt.printf(
		"%-24s text=\"%s\" caret=%d anchor=%d status=%d\n",
		label,
		text,
		int(gmte_get_caret(id)),
		int(gmte_get_anchor(id)),
		int(gmte_last_status()),
	)
}
...
main :: proc() {
	...
	fmt.println("GMTextEdit Odin self-test")
	fmt.println("Calling the same exported functions that GameMaker will bind to.")
	fmt.println()

	gmte_create(id, cstring("Hello"))
	print_editor("create", id)

	gmte_set_selection(id, gmte_get_text_length(id), gmte_get_text_length(id))
	gmte_input_text(id, cstring(", Odin"))
	print_editor("input text", id)

	gmte_command(id, cmd_real(.Select_Word_Left))
	selected := string(gmte_get_selected_text(id))
	fmt.printf("%-24s selected=\"%s\"\n", "select word left", selected)

	gmte_clipboard_set(cstring("GameMaker"))
	gmte_command(id, cmd_real(.Paste))
	print_editor("paste replacement", id)
